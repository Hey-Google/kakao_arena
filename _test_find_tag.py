...
for i in range(3):
    ...
    if i == 0:
        ...
        sp_A_const._shape = (int(g.playlist_meta_final.pid_code.max() + 1), int(g.test_tag_meta.tag_code.max() + 1))
        sp_A_const = sp_A_const.tocsr()
        sp_A_const_t = sp_A_const.T

        res = g.recs_for_ids(test_agg.pid_code.unique(), 'tag', origin, test_agg_pop, powb, sp_A, sp_A_const_t, knn_k, split, pid_codes, tag_codes, is_test = True)
